...
class PKMKValidation(ValidationStrategy):
    ...
    def validate(self, proposal: "ProposalContext") -> Tuple[bool, str]:
        ...
        filename_lower = proposal.file_content.lower()
        if not any(kw in filename_lower for kw in self.REQUIRED_KEYWORDS):
            return False, (
                f"Konten proposal tidak mengandung kata kunci 'anggaran' atau 'rab'. "
                f"PKM-K wajib menyertakan Rencana Anggaran Biaya (RAB)."
            )

        return True, (
            f"Konten proposal mengandung RAB. Validasi PKM-K berhasil."
        )
...
class PKMREValidation(ValidationStrategy):
    ...
    def validate(self, proposal: "ProposalContext") -> Tuple[bool, str]:
        ...
        if not any(kw in filename_lower for kw in self.REQUIRED_KEYWORDS):
            return False, (
                f"Konten proposal tidak mengandung bukti izin laboratorium. "
                f"PKM-RE wajib melampirkan surat izin penggunaan lab atau fasilitas riset."
            )

        return True, (
            f"Konten proposal mengandung bukti izin lab. Validasi PKM-RE berhasil."
        )
...
class PKMPMValidation(ValidationStrategy):
    ...
    def validate(self, proposal: "ProposalContext") -> Tuple[bool, str]:
        ...
        if not any(kw in filename_lower for kw in self.REQUIRED_KEYWORDS):
            return False, (
                f"Konten proposal tidak mengandung lampiran mitra. "
                f"PKM-PM wajib menyertakan surat kemitraan atau persetujuan dari mitra masyarakat."
            )

        return True, (
            f"Konten proposal mengandung lampiran mitra. Validasi PKM-PM berhasil."
        )
